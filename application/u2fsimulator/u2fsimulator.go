...
//	"x-mfa-RPOrigin"
//	"x-mfa-UserUUID"
func U2fRegistration(c *gin.Context) {
	challenge := c.PostForm("challenge")
	if challenge == "" {
		panic("'challenge' POST param is required.")
	}
	challenge = domain.FixStringEncoding(challenge)

	keyHandle := c.DefaultPostForm("keyHandle", DefaultKeyHandle)
	rpID := getRequiredHeader(c, "x-mfa-RPID")
	rpOrigin := getRequiredHeader(c, "x-mfa-RPOrigin")
	id := getRequiredHeader(c, "x-mfa-UserUUID")
...
func getRequiredHeader(c *gin.Context, key string) string {
	value := c.GetHeader(key)
	if value == "" {
		log.Fatalf("'%s' header is required.", key)
	}
	return value
}
